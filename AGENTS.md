...
1. **HomeHub redesign — DONE 2026-08-01 (`d3a3f4b`, refined in `656bbc4`); compact no-scroll pass 2026-08-07 (`52f6a19`).** The 08-07 pass replaced the two stacked grid rows (which forced scrolling to reach Workspaces/Shortcuts) with a single viewport-height 3-column flex layout (`h-[calc(100dvh-130px)]`, body grid `flex-1 min-h-0`) so everything is visible without scrolling on desktop. Columns: revenue hero + upcoming departures | KPI ledger + outstanding balances | workspaces + shortcuts. Tightened paddings, chart 88px, departures capped 4, payments 3. Verified no-scroll at 1280x577. The earlier Follow-up pass `656bbc4` fixed the gaps Ismail reported: the revenue hero and the metric ledger are now ONE panel (chart bleeds to the panel's bottom edge, ledger rows are hairline-divided and flush right), empty states are short muted lines instead of tall dashed slabs, and both grid columns are height-matched at 312px sidebar width. Verified in-browser light + dark, 1440px + mobile, BS + EN, console clean.
