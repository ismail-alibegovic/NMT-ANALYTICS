...
**Phase 0 — Production Stabilization: MERGED & COMPLETE (2026-08-27).**
- PR #2 (`stabilization-20260826`) merged into `main`. Merge commit `e0086068955ad71833b3d1f925f431ce9b842108`.
- Final stabilization HEAD: `d35c54ffcb33775d4add3cbd066d0a9fee003ebf`.
- Final CI run `33079533209` — ALL 5 JOBS GREEN: Lint & TypeCheck API PASS · Test API PASS (123/123) · Lint & Build Admin PASS · Test Admin PASS (40/40) · Docker Build Check PASS.
- Stabilization is accepted. CI now genuinely runs tests + Docker on every push (a local green is no longer trusted).

**CURRENT BRANCH: `feature/communication-center-2`** — created from `main @ e0086068955ad71833b3d1f925f431ce9b842108`. Clean starting point for Phase 1. No Phase 1 code written yet.

**NEXT TASK: PHASE 1 — COMMUNICATION CENTER 2.0 — NOT STARTED.** The detailed implementation prompt will be supplied separately; do not invent it.

### Resume rules
1. Do NOT continue from `stabilization-20260826` — it is merged and closed.
2. Do NOT reopen Phase 0 unless a real regression is discovered.
3. Work on `feature/communication-center-2`.
4. Production Stabilization is accepted and merged.
5. Preserve the stabilized architecture and invariants.
6. New Phase 1 work must be developed through the API-first architecture.
7. Do not modify live Supabase schema outside append-only migrations.
8. Do not create RPCs unless genuinely needed.
9. Every new feature must preserve explicit org/tenant scoping.
10. Do not start Phase 2 or later until Phase 1 is reviewed and accepted.

### Stabilization fixes (historical, now on `main` — do not redo)
- Passenger groups create 500 — invalid `accommodation_preference` default `prefer_together` (violated its own CHECK); fixed handler + column default → `no_preference` (+regression test).
- Add-member 404 / auto-group-on-sale — code referenced non-existent `trip_passenger_groups.reservation_id`; `primary_passenger_name` column was missing (migration adds it).
- "Razvrstavanje" grouping table rendered `undefined` — group rows are `{name,phone,partySize,roomType}`, not manifest shape; dedicated columns added.
- DepartureDetail workspace header hardcoded Bosnian (leaked into EN) — threaded through i18n (bs/en).
- Accommodation/rooming 500 PGRST201 — dual FK (`*_id_fkey` + composite `*_org_fk`) made PostgREST embeds ambiguous; disambiguated in `accommodation.ts` and `rooming.ts` (lines ~70/239).
- Known minor (non-blocking): full-room rejection surfaces as HTTP 500 from a DB CHECK/trigger instead of a clean 409 — capacity IS enforced, only the status code is coarse. Follow-up polish candidate.
...
**Auth reload-loop (P0) — FIXED 2026-08-26 (`1a92cb5`).** Root cause was NOT the module-level `isRedirectingToSignIn` guard from `0956b2f` — that guard resets on every full reload, and `0956b2f` still (a) called `fetchUserContext()`/`/me/context` while on `/auth/signin`, and (b) relied on a network `supabase.auth.signOut()` that left the stale token in localStorage if it failed, so each reload resurrected the 401 and re-redirected → cross-reload loop. Fix in `1a92cb5`: new `src/lib/authSession.ts` with (1) `isPublicAuthPath()` — never fetch protected data on `/auth/signin`; (2) synchronous `clearLocalAuthStorage()` before any async so a reload can't restore the dead token; (3) `handleExpiredSession()` as single owner of expiry (interceptor + AppContext + `api-auth-error` all funnel here, no competing navigations); (4) never navigate to the path you're already on; (5) sessionStorage-backed redirect budget (max 2 / 15s) that survives reloads. API `app.ts` sends `no-store` on index.html (hashed assets stay immutable) so a stale index can't pin an old bundle. Verified on the live URL with Playwright + real `admin@nmt.ba` login: fresh login succeeds → Dashboard 30s stable (0 navs, 0 401s) → Reservations/Departures/Dashboard nav clean → 2× hard-refresh stays logged in → expired-session redirects once, 0 protected API calls on `/auth/signin`, 0 page errors. admin+api `tsc` clean, 112/112 API tests pass, served bundle == disk, HEAD == origin/main.

0. **Travline secret standardization + service-role rotation — DONE 2026-08-17.** Ismail rotated the Travline Supabase `service_role` key and split Supabase secrets into project-prefixed Zo Secrets. Runtime verification: `TRAVLINE_SUPABASE_PROJECT_REF`, `TRAVLINE_SUPABASE_URL`, `TRAVLINE_SUPABASE_ANON_KEY`, `TRAVLINE_SUPABASE_SERVICE_ROLE_KEY`, and `TRAVLINE_SUPABASE_MANAGEMENT_TOKEN` are present; project ref/URL match `hacutwknfgufrqlgdiia`; Management API access returns 200; API health reports database connected. Code now prefers `TRAVLINE_*` env vars and keeps generic `SUPABASE_*` only as legacy fallback.
0. **Flights live schema/CRUD smoke — DONE 2026-08-17.** After applying `20260809010000_trip_type_hotel_specs_flights.sql`, verified `public.flights` with a temporary insert/select/delete smoke row (`Smoke Test Air` / `ZT999`) through the Supabase Management API. The row was deleted and post-delete count returned 0. API build/test/audit and admin build still pass. Browser unauthenticated access to `/operations/flights` correctly redirects to `/auth/signin`; authenticated visual testing still requires a Travline login session.
0. **Test/audit cleanup + settings schema drift guard — DONE 2026-08-16 (`ec8a2c8`, `57e5a45`, `8a13048`; migration applied live 2026-08-17).** Fixed the only failing API test suite (`pdfGeneration.test.ts`) by loading `pdf-parse` via CommonJS `require` instead of static ESM import; root cause was `pdf-parse@1.1.1` entering its broken sample-file debug branch and trying to read `./test/data/05-versions-space.pdf`. API tests now pass 12/12. `npm audit fix` bumped vulnerable transitive `nanoid` and `postcss`; API audit is back to 0 vulnerabilities. Also fixed `/settings` production schema drift: live DB lacked optional `organizations.tax_id`, so GET `/settings` logged `42703` and failed. Route now falls back to base organization columns when extended optional columns are absent, and migration `20260816010000_organization_settings_columns.sql` adds `timezone`, `tax_id`, `bank_account`, `invoice_footer`, `invoice_notes` for the permanent schema fix. Live DB verified on 2026-08-17: all 5 optional organization columns exist. Verification: API `tsc` clean, 12/12 tests pass, `npm audit` 0 vulnerabilities, admin production build passes, live service restarted, `/api/health` returns 200, service code up to date.
1. **HomeHub redesign — DONE 2026-08-01 (`d3a3f4b`, refined in `656bbc4`); compact no-scroll pass 2026-08-07 (`52f6a19`); visual consistency pass 2026-08-08 (`4514f2b`).** The 08-07 pass replaced the two stacked grid rows with a single viewport-height 3-column flex layout (`h-[calc(100dvh-130px)]`, body grid `flex-1 min-h-0`) so everything is visible without scrolling on desktop. The 08-08 pass (`4514f2b`) replaced the neumorphic design system (neu-shadow custom CSS, hex canvas colors, slate text, gradient buttons with glows, custom radii) with the flat bordered-card language used across all other Travline pages: `rounded-xl border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900` cards, `gray-*` text, solid `bg-brand-500` buttons, standard radii. Removed dead `neu-shadow-*` CSS from `index.css`. Layout (3-column compact no-scroll) preserved; only surface styling changed. Verified no-scroll + flat cards at 1280x577, light + dark. The earlier Follow-up pass `656bbc4` fixed the gaps Ismail reported: the revenue hero and the metric ledger are now ONE panel (chart bleeds to the panel's bottom edge, ledger rows are hairline-divided and flush right), empty states are short muted lines instead of tall dashed slabs, and both grid columns are height-matched at 312px sidebar width. Verified in-browser light + dark, 1440px + mobile, BS + EN, console clean.
   Note: the `www.google-analytics.com/mp/collect` `ERR_BLOCKED_BY_CLIENT` errors Ismail saw are NOT from Travline — grepped the whole repo, `dist/`, and the served HTML for `G-03XW3FWG7L`/`gtag`/`googletagmanager`: zero hits. It's a browser extension or another open tab; the ad-blocker is blocking it, which is why it's `BLOCKED_BY_CLIENT` and not a real failure.
   Original redesign (`d3a3f4b`): Rebuilt the hub around one revenue hero (big paid-revenue number + real second-half-vs-first-half trend delta + area chart) and a single "metric ledger" card (dividers, not four floating KPI boxes) that also holds the Workspaces nav. Row B keeps Upcoming departures (left) and Outstanding balances + Shortcuts (right). Existing icon set, `brand-500` accent, `rounded-2xl` panels, and typography scale all preserved — nothing new introduced visually. New i18n keys: `hub.trendVsPrev`, `hub.quietPeriod`, `hub.allSettled` (bs + en). Verified in the browser at 1280px and 420px, light + dark, BS + EN, with a local API on port 3011 and dev-login.
2. **UX redesign (Passes A/B/C) — REVERTED 2026-08-01 (`4c78367`).** Passes A/B/C (PageShell frame, DataTable sort + row-click, HomeHub range control + Needs Attention worklist) were implemented across commits `4d63aac`, `8f625fe`, `818d76c`, `e7a0bd3` and then **fully reverted** — Ismail's verdict: "samo si pokvario izgled, sada je sve poremeceno i razbijeno, nije poredano lijepo, brojevi i dosta stvari vise nije kako treba". The admin UI is now byte-identical to `36f465f`; only Sentry (`b573e98`) sits on top.
   **Lesson for the next attempt:** do NOT do broad mechanical refactors across 20+ pages behind a `tsc`/`vite build` green light. The builds passed while spacing, alignment, and number formatting visibly regressed. Any future UX work must be (a) one page at a time, (b) visually diffed against the current live page in the browser before and after, and (c) shown to Ismail for approval before touching a second page. `TRAVLINE_UX_REDESIGN_PLAN.md` is now HISTORICAL, not active.
3. **Sentry activation — DONE 2026-07-31 (`b573e98`).** Wired end-to-end and DSN-gated: `nmt-analytics-api/src/instrument.ts` (must stay the first import in the API process), tenant-scoped tags (`tenant.org_id` + `user.role`), PII-free, 4xx pass through untouched, only real 5xx captured. `apiClient` + `ErrorBoundary` report from the admin side. `/api/health` now exposes `config.sentryEnabled`. To turn on: set `SENTRY_DSN` (API) and `VITE_SENTRY_DSN` (admin — baked at build time, so it needs a rebuild, not just a restart) in [Settings > Advanced](/?t=settings&s=advanced).
4. **Sprint 4 — Stripe Billing** (`TRAVLINE_FINAL_PLAN.md` §4) — DEFERRED per Ismail's directive ("do not build yet, until first paying client"). Reconfirmed skipped 2026-07-31.
5. **F-2 — Rotate the Supabase `service_role` key — DONE 2026-08-17.** Ismail rotated the key and updated the project-prefixed Zo Secret `TRAVLINE_SUPABASE_SERVICE_ROLE_KEY`.
...
Both projects `tsc` clean, `npm audit` 0 vulnerabilities, API tests 12/12 pass. The old `pdfGeneration.test.ts` failure was resolved 2026-08-16 in commit `ec8a2c8` by loading `pdf-parse` through CommonJS `require`, avoiding its broken import-time sample-file branch. Pushed to `main`; live API restarted and `/api/health` returns 200. Timeline: minimum demoable in ~1.5 weeks (~8 working days), polished production in ~3 weeks (~15 working days).
...
1. **Rotate the Supabase `service_role` key — DONE 2026-08-17.** Stored under `TRAVLINE_SUPABASE_SERVICE_ROLE_KEY`; generic `SUPABASE_*` names are legacy fallback only.
...
https://travline-sprypine.zocomputer.io
