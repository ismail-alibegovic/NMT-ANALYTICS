-- Atomic rooming proposal apply RPC for M10.2.
-- Replaces only non-manual non-locked auto assignments in a single transaction.
-- Never overwrites manual (is_manual=true) or explicitly locked assignments.
...
BEGIN
  ...
  -- Delete only auto+unlocked replaceable assignments.
  -- If any requested ID is now manual or locked, abort the whole operation.
  IF p_replaceable_assignment_ids IS NOT NULL AND array_length(p_replaceable_assignment_ids, 1) > 0 THEN
    ...
    WHERE a.id = ANY(p_replaceable_assignment_ids)
      ...
      AND (a.is_manual = true OR a.locked = true);
    IF FOUND THEN
      RAISE EXCEPTION 'ROOM_ASSIGNMENT_LOCKED';
    END IF;
  ...
  FOR proposed_item IN SELECT * FROM jsonb_array_elements(p_proposed) LOOP
    ...
    v_full_name := pax_row.full_name;
    v_reservation_id := pax_row.reservation_id;

    -- Resolve canonical accommodation requirement
    ...
    -- Check passenger is not already assigned (fixed or already-proposed)
    ...
    -- Check slot capacity (counts all current assignments including already-inserted proposed)
    ...
    VALUES (
      ...
      v_full_name, p_assigned_by, false, false
    );
