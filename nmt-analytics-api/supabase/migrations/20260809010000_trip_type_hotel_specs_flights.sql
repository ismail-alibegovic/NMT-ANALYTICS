-- Trip types, hotel specifications, and flight catalog
-- Adds: packages.trip_type + tags, hotel specs (stars/description/amenities/email/website),
--       flights table, departures.flight_id

-- 1. Packages: trip type + tags ------------------------------------------------
ALTER TABLE packages
  ADD COLUMN IF NOT EXISTS trip_type TEXT NULL DEFAULT 'other'
    CHECK (trip_type IN ('beach','city','pilgrimage','honeymoon','ski','adventure','cruise','cultural','wellness','other')),
  ADD COLUMN IF NOT EXISTS tags JSONB NULL DEFAULT '[]';
...
COMMENT ON COLUMN packages.tags IS 'Secondary tags array (free-form strings for filtering)';

-- 2. Hotels: specifications ----------------------------------------------------
ALTER TABLE hotels
...
COMMENT ON COLUMN hotels.amenities IS 'JSON array of amenity strings: wifi, pool, spa, gym, restaurant, parking, etc.';
...
COMMENT ON COLUMN flights.departure_airport IS 'IATA airport code (e.g. SJJ, IST, VIE)';
COMMENT ON COLUMN flights.arrival_airport IS 'IATA airport code (e.g. SJJ, IST, VIE)';
